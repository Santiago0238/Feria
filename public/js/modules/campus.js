...
const CampusModule = {
    ...
    cargarPuntosDesdeSupabase: async function() {
        ...
        try {
            const { data, error } = await db
                ...
                .select('id, name, desc_info, lat, lng, points, type_material, icon_name,codigo_qr');
   ...
   openQrScanner: async function() {
    ...
    // CAMBIO CLAVE: Ahora compara contra la columna de la base de datos, no contra el ID
    const codigoRequerido = String(this.activePoint.qrEsperado).trim().toLowerCase();
    console.log(`[Seguridad Avanzada] QR Escaneado: "${valorCrudo}" | Código requerido en DB: "${codigoRequerido}"`);

    return valorCrudo === codigoRequerido;
},
